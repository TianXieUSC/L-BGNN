...
# create the data loader for batch training
X_train_one = X_one[train_mask]
X_train_two = X_two[train_mask]
y_train = label[train_mask]
feeder_train = Feeder(X_train_one, X_train_two, y_train)
dataset_train = torch.utils.data.DataLoader(dataset=feeder_train, batch_size=256, shuffle=False, drop_last=False)

X_val_one = X_one[val_mask]
X_val_two = X_two[val_mask]
y_val = label[val_mask]
if torch.cuda.is_available():
    X_val_one = X_val_one.cuda()
    X_val_two = X_val_two.cuda()
    y_val = y_val.cuda()
...
for i in range(model_args['epoch']):
    ...
    for batch in dataset_train:
        ...
        batch_label = batch[2]
        if torch.cuda.is_available():
            batch_one = batch_one.cuda()
            batch_two = batch_two.cuda()
            batch_label = batch_label.cuda()
        optimizer.zero_grad()
        y_prob = model(batch_one, batch_two)
        train_loss = F.binary_cross_entropy(y_prob, batch_label)

        train_loss.backward()
        optimizer.step()
        train_acc = accuracy(y_prob, batch_label)

    # validation
    y_prob_val = model(X_val_one, X_val_two)
    val_loss = F.binary_cross_entropy(y_prob_val, y_val)
    val_acc = accuracy(y_prob_val, y_val)
    ...
    print('epoch: {}, '
          'val loss: {:.4f}, val accuracy: {:.4f}'.format(i, val_loss, val_acc))
